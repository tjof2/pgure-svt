        ...
        void Estimate(const arma::cube &A,
            ...
            if (iter < timewindow) {
                ...
                for (int i = 0; i < 2*timewindow - endseqFrame; i++) {
                    ARPSMotionEstimation(A, i, endseqFrame+i,
                                               endseqFrame+i+1,
                                               endseqFrame+i);
                }
                // Go backwards
                for (int i = -1; i >= -endseqFrame; i--) {
                    if (2*timewindow == endseqFrame) {
                        ARPSMotionEstimation(A, i, endseqFrame+i+1,
                                                   endseqFrame+i,
                                                   endseqFrame+i);
                    } else {
                        ARPSMotionEstimation(A, i, endseqFrame+i+1,
                ...
                for (int i = 0; i < timewindow; i++) {
                    ARPSMotionEstimation(A, i, timewindow+i,
                                               timewindow+i+1,
                                               timewindow+i);
                }
                // Go backwards
                for (int i = -1; i >= -timewindow; i--) {
                    ARPSMotionEstimation(A, i, timewindow+i+1,
                                               timewindow+i,
                                               timewindow+i+1);
                }
            }
        ...
        void ARPSMotionEstimation(const arma::cube &A,
            ...
            for (int it = 0; it < vecSize; it++) {
                ...
                arma::cube newblock = A(arma::span(i, i+Bs-1),
                                        arma::span(j, j+Bs-1),
                                        arma::span(iARPS2));
                costs(2) = std::pow(arma::norm(
                                refblock.slice(0) - newblock.slice(0), "fro"),
                             2) / (Bs*Bs);
                chkMat(wind, wind) = 1;
                ...
                for (int k = 0; k < maxIdx; k++) {
                    ...
                    if (refBlkHor < 0
                        ...
                        arma::cube powblock = A(arma::span(refBlkVer,
                                                           refBlkVer+Bs-1),
                                                arma::span(refBlkHor,
                                                           refBlkHor+Bs-1),
                                                arma::span(iARPS2));
                        if (curFr == 0) {
                            costs(k) = std::pow(arma::norm(refblock.slice(0)
                                                 - powblock.slice(0), "fro"),
                                             2) / (Bs*Bs);
                        } else if (curFr < 0) {
