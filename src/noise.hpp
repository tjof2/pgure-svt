...
class NoiseEstimator {
                ...
                case 1:
                    {
                        ...
                        mu = (mu >= 0.) ? mu : ComputeMode(
                                   RestrictArray(rmeans, 0, std::round(0.05*L))
                              );
                        dSi = ComputeMode(
                                   RestrictArray(rvars, 0, std::round(0.05*L))
                              );
                ...
                case 3:
                    {
                        ...
                        mu = (mu >= 0.) ? mu : ComputeMode(rmeans);
                        sigma = (sigma >= 0.) ? sigma : std::sqrt(
                                    std::abs(alphaBeta(1)+alphaBeta(0)*mu)
                                    );
                        break;
                    }
                case 4:
 ...
 private:
        ...
        const arma::vec Ftest0100 = {5.39077, 1.97222, 1.38391,
                                     1.17439, 1.08347, 1.04087,
                                     1.02023, 1.01006, 1.00502,
                                     1.00251, 1.00125, 1.00063};
        const arma::vec Ftest0050 = {9.27663, 2.40345, 1.51833,
                                     1.22924, 1.10838, 1.05276,
                                     1.02604, 1.01293, 1.00645,
                                     1.00322, 1.00161, 1.00081};
        const arma::vec Ftest0025 = {15.4392, 2.86209, 1.64602,
                                     1.27893, 1.13046, 1.06318,
                                     1.03110, 1.01543, 1.00769,
                                     1.00384, 1.00192, 1.00096};
        const arma::vec Ftest0010 = {29.4567, 3.52219, 1.80896,
                                     1.33932, 1.15670, 1.07543,
                                     1.03702, 1.01834, 1.00913,
                                     1.00455, 1.00227, 1.00114};
        ...
        void WeightFunction(const arma::vec &x, arma::vec &w) {
            ...
            switch (wtype) {
                ...
                default:
                    ...
                    pp = p*p;
                    for (size_t i = 0; i < x.n_elem; i++) {
                        w(i) = (std::abs(x(i)) > p) ? 0. : (pp
                                    - x(i)*x(i))*(pp - x(i)*x(i)) / (pp*pp);
                    }
                    break;
            }
            return;
        ...
        void QuadTree(const arma::mat &A,
                      int part) {
            ...
            arma::mat patch = A.submat(arma::span(i, i+s-1),
                                       arma::span(j, j+s-1));
            ...
            newtreeadd << i << i+s << i << i+s << arma::endr
                       ...
                       << s << s << s << s << arma::endr;
            arma::umat newtree = arma::join_horiz(
                                    treeDelete[0], newtreeadd);

            arma::umat newdeleteadd(1,1);
            newdeleteadd << part << arma::endr;
            arma::umat newdelete = arma::join_horiz(
                                    treeDelete[1], newdeleteadd);
